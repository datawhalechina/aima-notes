...
> 参考答案：
> 1. 在此环境下一个简单反射型智能体不完全合理。因为这个智能体永远不会停止，它的得分也会不断下降。
> 2. 带有状态的反射性智能体是可能的，只要智能体对区域和环境有记忆，就有可能工作。否则它会不断地从一个地方移动到另一个地方。（智能体设计时只要说明“在所有方块都干净后停止”即可）
> 3. 如果这个智能体知道这个空间是清洁/肮脏的，它可以选择不采取任何措施以防止分数继续降低。智能体应该只在肮脏的区域内进行打扫。如果要前往肮脏的区域，应该走最短路径，从而是分数降低的程度最低。
...
> 参考答案：
> 这个问题乍一看非常相似;主要区别在于，智能体不必使用位置感知来构建地图，而是必须“发明”自己的位置(毕竟，这些位置只是表示状态空间图的数据结构中的节点)。当检测到一个碰撞时，代理认为它仍然在相同的位置，并可以在地图上添加一面墙。
对于网格环境，代理可以跟踪它的(x,y)位置，因此可以判断它何时返回到旧状态。然而，在一般情况下，没有简单的方法来判断一个状态是新的还是旧的。
